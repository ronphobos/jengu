...
class Products
{
	...
	public function getProducts(){
		$q = $this->con->query("SELECT p.produit_id, p.produit_title, p.produit_image, p.produit_keywords, c.cat_title, c.cat_id, b.brand_id, b.brand_title FROM info_products p JOIN categories_info c ON c.cat_id = p.produit_cat JOIN brand_info b ON b.brand_id = p.produit_brand");
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/jengu/product_images/".$uniqueImageName)) {
					...
					$q = $this->con->query("INSERT INTO `info_products`(`produit_cat`, `produit_brand`, `produit_title`, `produit_image`, `produit_keywords`) VALUES ('$category_id', '$brand_id', '$product_name', '$uniqueImageName', '$product_keywords')");
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/jengu/product_images/".$uniqueImageName)) {
					...
					$q = $this->con->query("UPDATE `info_products` SET 
										`produit_cat` = '$category_id', 
										`produit_brand` = '$brand_id', 
										`produit_title` = '$product_name', 
										`produit_image` = '$uniqueImageName', 
										`produit_keywords` = '$product_keywords'
	...
	public function deleteProduct($pid = null){ 
		if ($pid != null) {
			$q = $this->con->query("DELETE FROM info_products WHERE produit_id = '$pid'");
			if ($q) {
